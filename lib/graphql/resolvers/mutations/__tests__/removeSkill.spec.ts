...
test('removes single skill in mydata', async () => {
  await removeSkill({}, args, ctx, {} as any)
...
test('returns true on success', async () => {
  ...
  await expect(removeSkill({}, args, ctx, {} as any)).resolves.toEqual(true)
})

test('handles errors', async () => {
  ctx.mydata.removeData.mockRejectedValue('err')

  await expect(removeSkill({}, args, ctx, {} as any)).rejects.toThrow('err')
})
